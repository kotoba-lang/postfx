(ns postfx
  ...
  This file was deleted on 2026-07-20 by `15c7fda2` (\"Migrate post-effect
  configuration to Kotoba\"), which put `src/postfx.kotoba` at the same path. That
  left the `postfx` namespace unloadable by every Clojure runtime -- while
  `kotoba-lang/kami-postfx-scene` still requires it on its production path
  (`src/postfx_scene.cljc:61`). That consumer only kept building because its
  `deps.edn` pinned this repo four commits BEFORE the migration; advancing the pin
  would have broken it.

  `src/postfx.kotoba` remains the SEMANTIC AUTHORITY and is not reverted. This
  `.cljc` is the LOAD PATH, and `test/postfx_parity_test.clj` asserts the two agree
  by compiling the `.kotoba` and running it through the KIR interpreter in the same
  JVM (the shape `kotoba-lang/css`, `kotoba-lang/dsl-core` and `kotoba-lang/async`
  use). Change one, change the other; the gate will say so.

  Two deliberate differences from the guest, both stated in the parity test:

  * `effect-types` is a `def`'d SET here and a nullary function there. Kotoba has no
    top-level value bindings, so the guest can only expose it as `(effect-types)`.
    The members are identical and the gate asserts that.
  * The guest's `main` (the wasm entry point, `(document-count …)` of the nintendo
    preset) is not part of this library's API and is not mirrored, following
    `dsl-core`.

  Removal condition: when consumers have a load path that does not need a `.cljc`
  (for the native route, ADR-2607279200 W4). Until then, deleting this file is not a
  migration step, it is an outage.")
